...
import GoogleIcon from '@mui/icons-material/Google'; import { IconButton, Modal } from "@mui/material";
...
const SignUp = ({ setSignUpOpen, setSignInOpen }) => {
  ...
  const createAccount = () => {
    if (otpVerified) {
      ...
      setLoading(true);
      try {
        signUp({ name, email, password,confirmPassword }).then((res) => {
          if (res.status === 200) {
  ...
  const handleSignUp = async (e) => {
    ...
    const response = await fetch(`http://localhost:5000/api/auth/userExist`, {
      method: 'POST',
      headers: {
